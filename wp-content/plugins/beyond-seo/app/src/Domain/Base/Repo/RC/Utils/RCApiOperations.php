...
class RCApiOperations
{
    ...
    private function callRCApi(
        ...
        $resolvedTimeout = $timeout > 0 ? $timeout : ($callRCRequestConfig['timeout'] ?? 30);
        $monolithCalls = [];
        $responses = [];
        ...
        foreach ($callData as $endpoint => $parameters) {
            ...
            if (isset($matches['url']) && isset($matches['method'])) {
                ...
                foreach ($parameters as $parametersAndCallId) {
                    ...
                    $wpHeaders['rc-tracking'] = 'accountId=' . ($accountId??'') . ' calledFrom=' . $routeCommandOrMessageHandlerAndTraceAsString;
                    $wpArgs = [
                    ...
                    if (isset($parametersAndCallId['body'])) {
                        $wpArgs['body'] = $parametersAndCallId['body'];
                    }
                    $callKey = $endpoint . $endpointToIdDelimiter . $parametersAndCallId['id'];
                    $wpResponse = wp_remote_request($callUrl, $wpArgs);
                    if (is_wp_error($wpResponse)) {
                        $exceptionDetails = new ExceptionDetails();
                        $exceptionDetails->addDetail($wpResponse->get_error_message(), debug_backtrace());
                        throw new InternalErrorException('API request failed', $exceptionDetails);
                    }
                    $responses[$callKey] = $wpResponse;
                    if (RCLoad::$logRCCalls) {
                        self::$executedRCCalls[$callKey] = ['call' => $parameters];
                    }
                }
            } else {
        ...
        if ($monolithCalls) {
            ...
            $monolithWpResponse = wp_remote_post(
                $monolithConfig['baseUrl'],
                ['body' => $monolithCallData, 'timeout' => $resolvedTimeout]
            );
            if (is_wp_error($monolithWpResponse)) {
                $exceptionDetails = new ExceptionDetails();
                $exceptionDetails->addDetail($monolithWpResponse->get_error_message(), debug_backtrace());
                throw new InternalErrorException('Monolith API request failed', $exceptionDetails);
            }
            $responses['monolith'] = $monolithWpResponse;
            if (RCLoad::$logRCCalls) {
        ...
        foreach ($responses as $endpointAndId => $response) {
            ...
            if ($endpointAndId !== 'monolith') {
                ...
                $monolithReponse = json_decode(wp_remote_retrieve_body($response));
