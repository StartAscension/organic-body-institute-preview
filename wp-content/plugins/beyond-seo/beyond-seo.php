...
/**
 * Dual-activation safety net: if the new `beyondseo` plugin is already active,
 * deactivate this legacy plugin immediately and bail out, to avoid fatal
 * "cannot redeclare" errors from shared namespaces/functions/DB tables.
 * This must run using only WordPress core functions, before any `inc/`
 * class is loaded.
 */
$rankingcoach_new_plugin_basename = 'beyondseo/beyondseo.php';

if ( ! function_exists('is_plugin_active') ) {
    require_once ABSPATH . 'wp-admin/includes/plugin.php';
}

if ( is_plugin_active($rankingcoach_new_plugin_basename) || ( is_multisite() && is_plugin_active_for_network($rankingcoach_new_plugin_basename) ) ) {
    deactivate_plugins(plugin_basename(__FILE__), true);
    return;
}
unset($rankingcoach_new_plugin_basename);
