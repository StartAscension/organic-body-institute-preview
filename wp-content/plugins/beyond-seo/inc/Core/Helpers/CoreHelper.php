...
class CoreHelper {
	...
	/**
	 * Allow-list of subscription plan names that are permitted to see the upsell page.
	 *
	 * The upsell page (and all its entry points: menu item, toolbar link, plugin
	 * action link, admin notifications and dashboard widget) is only shown when the
	 * current subscription's plan_name is in this list. Any plan NOT listed here
	 * (including unknown plans) will NOT see the upsell page.
	 *
	 * To enable/disable the upsell for a plan, simply add or remove its plan_name
	 * (the rankingCoach plan code, case-insensitive) below.
	 */
	public const UPSELL_ALLOWED_PLANS = [
		'seo_wp_ai_visibility',
		'seo_wp_ai_reputation',
	];
    ...
    public static function is_paid($returnNameIfTrue = false): bool|string
    {
        ...
        if ( is_array( $subscription ) ) {
            ...
            try {
                $decoded = json_decode($subscription, true, 512, JSON_THROW_ON_ERROR);
                if ( is_array( $decoded ) ) {
                ...
                // If JSON decoding fails, treat it as a direct plan name
                $planName = $subscription;
            }
        }
        ...
        if (!empty($subscription)) {
            ...
            if (is_string($subscription)) {
                if (self::is_valid_json($subscription)) {
                    $subscription = json_decode($subscription, true, 512, JSON_THROW_ON_ERROR);
                } else {
    ...
    /**
     * Determine whether the upsell page should be available for the current subscription.
     *
     * The upsell page is only shown when the current subscription's plan_name is in the
     * UPSELL_ALLOWED_PLANS allow-list. Any plan not in the list (including no subscription,
     * unknown plans, or the highest tier) will NOT see the upsell page.
     *
     * @return bool True if the current plan is allow-listed for upsell, false otherwise.
     */
    public static function isUpsellAllowed(): bool
    {
        ...
        // Multi-packet model: if the customer owns one or more plan packets, keep the
        // upsell page available so they can purchase the packets they don't have yet.
        if (!empty(self::getActiveSubscriptions())) {
            return true;
        }
        ...
        $currentPlanName = strtolower(trim($subscription['plan_name']));
        $allowedPlans = array_map('strtolower', self::UPSELL_ALLOWED_PLANS);

        return in_array($currentPlanName, $allowedPlans, true);
    }

    /**
     * Temporary seed of owned packets used while the real data is not yet written to
     * the options table. Remove this once the owned packets are populated from the API.
     *
     * @var string[]
     */
    public const TEMP_DEFAULT_OWNED_PACKETS = [
        'momentum_search',
        'momentum_reputation',
		// 'momentum_marketing',
    ];
    ...
    /**
     * Determine whether the current customer has an active subscription for a specific plan.
     *
     * @param string $packet The subscription name to check (case-insensitive).
     * @return bool True if an active subscription exists, false otherwise.
     */
