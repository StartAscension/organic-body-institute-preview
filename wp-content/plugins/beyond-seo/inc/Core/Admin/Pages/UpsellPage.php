...
class UpsellPage extends AdminPage
{
    ...
    {
        // Only render the upsell page for allow-listed subscription plans.
        // Defensive guard: the menu page is only registered when allowed, but this
        // ensures the page can never render for a non-allow-listed plan via any route.
        if (!CoreHelper::isUpsellAllowed()) {
            wp_safe_redirect(AdminManager::getPageUrl(AdminManager::PAGE_MAIN));
            exit;
        }
        ...
        try {
            ...
            $this->log('Token validation failed in UpsellPage: ' . $e->getMessage(), 'ERROR');
            ...
            // Redirect based on channel to re-activate/re-register
            $nextPage = ($channel === 'ionos' || $channel === 'extendify')
                ? AdminManager::PAGE_ACTIVATION
                : AdminManager::PAGE_REGISTRATION;

            wp_safe_redirect(AdminManager::getPageUrl($nextPage));
            exit;
        }
