...
use RankingCoach\Inc\Core\Jobs\CronJobClass;
...
class HooksManager
{
    ...
    public function pluginLoaded(): void
    {
        ...
        if (WordpressHelpers::isOnboardingCompleted()) {
            ...
            // registerCronScheduleListeners() and Installer.
    ...
    public function pluginUpgraderProcessComplete(WP_Upgrader $upgrader, array $hook_extra = []): void
    {
        ...
        if (($hook_extra['action'] ?? '') === 'update' && ($hook_extra['type'] ?? '') === 'plugin') {
            $plugin_basename = 'beyond-seo/beyond-seo.php';
    ...
    public function maybeNotifyWpCronDisabled(): void
    {
        try {
            ...
            $message = sprintf(
                '<p>%s</p>',
                __('WordPress Cron is disabled on this site (DISABLE_WP_CRON is set to true). RankingCoach relies on WP-Cron to run scheduled tasks such as keyword synchronization and broken link checking. Please ensure a system cron job is configured to run wp-cron.php periodically, otherwise these scheduled tasks will not run.', 'beyond-seo')
            );
