...
class TokensManager
{
	...
	public const ACCESS_TOKEN_EXPIRES_AT = BaseConstants::OPTION_ACCESS_TOKEN_EXPIRES_AT;
	public const REFRESH_TOKEN = BaseConstants::OPTION_REFRESH_TOKEN;

	/**
	 * Request-level caching for tokens.
	 */
	private static ?string $cachedAccessToken = null;
	private static ?string $cachedRefreshToken = null;
	private static ?int $cachedExpiresAt = null;
	 ...
	 * @return bool
	 */
	public static function updateTokens( mixed $accessToken, mixed $refreshToken, ?int $expiresAt = null ): bool {
		if ( $expiresAt === null ) {
			list( , $expiresAt ) = self::getJwtParts( $accessToken );
		}
		...
		$updated = update_option( self::ACCESS_TOKEN, $accessToken ) &&
			update_option( self::REFRESH_TOKEN, $refreshToken );

		if ( $expiresAt ) {
			update_option( self::ACCESS_TOKEN_EXPIRES_AT, $expiresAt );
		}
    ...
    public function deleteTokens(): bool {
		...
		self::$cachedRefreshToken = null;
		self::$cachedExpiresAt = null;

        return
            delete_option( self::ACCESS_TOKEN ) &&
			delete_option( self::ACCESS_TOKEN_EXPIRES_AT ) &&
	...
	public static function validateToken( ?string $token = null,  bool $returnExpiration = false ): bool|int {
		...
		// 1. Check cache first
		if ( $token === null || $token === self::$cachedAccessToken ) {
			$expireAt = self::$cachedExpiresAt;
		}

		// 2. Check option if no cache
		if ( ! $expireAt && ( $token === null || $token === get_option( self::ACCESS_TOKEN ) ) ) {
			$expireAt = (int) get_option( self::ACCESS_TOKEN_EXPIRES_AT );
		}

		// 3. Fallback to JWT decoding if still no expiration found
		if ( ! $expireAt ) {
	...
	public function getStoredAccessToken(): string {
		if ( self::$cachedAccessToken === null ) {
			self::$cachedAccessToken = (string) get_option( self::ACCESS_TOKEN, '' );
		}
	...
	public function getStoredRefreshToken(): string {
		if ( self::$cachedRefreshToken === null ) {
			self::$cachedRefreshToken = (string) get_option( self::REFRESH_TOKEN, '' );
		}
		return self::$cachedRefreshToken;
	}
